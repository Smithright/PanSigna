"""Build a bounded v0.3 report from complete run artifacts; refuse partial results."""
import argparse
import hashlib
import json
...
def build(runs, out):
    ...
    a = [read(runs/'v03A'/'aligned'/f'{arm}-seed{seed}'/'metrics.json')
         for arm in ('text','atomic','bits') for seed in (3,11,29)]
    random = [read(runs/'v03A'/'randomized'/f'{arm}-seed3'/'metrics.json')
              for arm in ('text','atomic','bits')]
    b = [read(runs/'v03B'/'calibration'/f'{arm}-seed41'/'metrics.json')
         for arm in ('text','sense','atomic')]
    readbacks = {arm:read(runs/'v03B'/'calibration'/f'{arm}-seed41'/'checkpoint-readback.json')
                 for arm in ('text','sense','atomic')}
    assert all(r['weights_unchanged'] for r in a+random)
    assert all(r['code_sha256']==sha(Path('src/pansigna/alignment.py')) for r in a+random)
    assert all(r['config']['source_sha256']==sha(Path('src/pansigna/graphs.py')) for r in b)
    summary = dict(phase_a=a, randomized=random, phase_b=b, checkpoint_readbacks=readbacks, source_files=inputs)
    (out/'summary.json').write_text(json.dumps(summary,indent=2)+'\n')
    lines = ['# PanSigna v0.3: causal alignment and graph calibration', '',
    ...
    for r in a:
        t=r['tests']['crossed']
        values=[f"{t[k]['recipient_success']}/48" for k in ('none','aligned','ridge','random','shuffled_donor','full_state')]
        lines.append('| '+ ' | '.join([r['arm'],str(r['seed']),*values,f"{t['aligned']['other_correct']}/96"])+' |')
    lines += ['', 'Minimal donors change only the recipient:', '',
              '| Arm | Seed | Aligned swaps | Counterfactual log margin before → after |',
              '|---|---:|---:|---:|']
    for r in a:
        t=r['tests']['minimal']
        lines.append(f"| {r['arm']} | {r['seed']} | {t['aligned']['recipient_success']}/48 | {t['none']['recipient_margin']:.3f} → {t['aligned']['recipient_margin']:.3f} |")
    lines += ['',
        ...
        '|---|---:|---:|---:|---:|---:|---:|---|']
    def score(m,f):
        return f"{m[f]['correct']}/{m[f]['n']} ({100*m[f]['correct']/m[f]['n']:.1f}%)"
    for r in b:
        dev=r['development'][-1]['metrics']
        lines.append('| '+' | '.join([r['config']['arm'],str(r['steps']),str(r['parameters']),
                    score(dev,'follow2'),score(dev,'next'),score(r['test'],'follow2'),score(r['test'],'next'),
                    'pass' if r['competence_gate_pass'] else 'fail'])+' |')
    lines += ['', '| Arm | Test legible | Test terminated | Unseen surface two-step | Unseen surface one-step | Input token exposures | Seconds |',
              '|---|---:|---:|---:|---:|---:|---:|']
    for r in b:
        lines.append('| '+' | '.join([r['config']['arm'],f"{r['test']['legible']}/640",f"{r['test']['terminated']}/640",
                    score(r['unseen_surface'],'follow2'),score(r['unseen_surface'],'next'),str(r['tokens_seen']),f"{r['seconds']:.1f}"])+' |')
    lines += ['', 'The atomic unseen-template input collapses to the same IR as the seen',
        ...
        'Shortcut baselines on 640 test queries: '+', '.join(f"{k} {v['correct']}/{v['n']}" for k,v in b[0]['shortcut_baselines'].items())+'.', '',
    ...
    if not any(r['competence_gate_pass'] for r in b):
        lines += ['**Gate outcome:** all arms failed ordinary task competence. We therefore',
        ...
        lines += ['At least one arm passed development competence. Intermediate causal',
                  'interventions require their own subsequent evaluation; this report',
                  'contains ordinary-task calibration only.', '']
    lines += ['Post-hoc checkpoint readback reproduced all 640 saved test generations',
        'for each graph arm. Training-set competence (no further fitting or selection):', '',
        '| Arm | Training two-step | Training one-step |',
        '|---|---:|---:|']
    for arm, r in readbacks.items():
        lines.append(f"| {arm} | {score(r['train'],'follow2')} | {score(r['train'],'next')} |")
    lines += ['', 'Atomic PanSigna fits these training examples much better, yet fails held-out',
    ...
    lines += ['', '## Next rung and claim boundary', '',
